...
public class InGameMenu extends JPanel {
	...
	public InGameMenu(PokemonGame model, GameGUI gui) {
		...
		spacer.setPreferredSize(new Dimension((int) buttonSize.getWidth(), (int) (buttonSize.getHeight() / 3)));
		contents.add(spacer);
		...
		mapOne = new JButton("Go To Map One", new ImageIcon(this.getClass().getResource(buttonPNG)));
		...
		mapTwo = new JButton("Go To Map Two", new ImageIcon(this.getClass().getResource(buttonPNG)));
		...
		save = new JButton("Save Game", new ImageIcon(this.getClass().getResource(buttonPNG)));
		...
		resume = new JButton("Resume", new ImageIcon(this.getClass().getResource(buttonPNG)));
		...
		quit = new JButton("Quit", new ImageIcon(this.getClass().getResource(buttonPNG)));
	...
	public void paintComponent(Graphics g) {
		...
		g2.drawString(str, this.getWidth() / 2 - metricsHdr.stringWidth(str) / 2, 50 + metricsHdr.getHeight() / 2);
		...
		g2.setColor(Color.WHITE);
		g2.setFont(fontMap);
		g2.drawString(mapStr, this.getWidth() / 2 - metricsMap.stringWidth(mapStr) / 2,
	...
	private void drawPokedex(Graphics2D g2) {
		...
		g2.drawString("Pokedex (" + pokeList.size() + " Caught)", x + 15, y - 5);
		...
		Image emptyTile = new ImageIcon(this.getClass().getResource("/art/on_screen_icons/pokemon_empty.png"))
				.getImage();
		...
		for (int r = 0; r < rows; r++) {
			for (int c = 0; c < cols; c++) {
				g2.drawImage(emptyTile, x + c * width, y + r * height, width, height, null, null);

				if (p < pokeList.size()) {
					pokes[r][c] = pokeList.get(p);
					final int offset = 5;
					g2.drawImage(pokes[r][c].getSprite(), offset + x + c * width, offset + y + r * height,
							width - offset * 2, height - offset * 2, null, null);
				}
				p++;
			}
		}
		...
		Image statsBox = new ImageIcon(this.getClass().getResource("/art/on_screen_icons/poke_stats.png")).getImage();
		g2.drawImage(statsBox, statsX, statsY, width * cols, height + 20, null, null);

		if (this.selectedPoke != null) {
			g2.setColor(Color.LIGHT_GRAY);
			g2.setFont(new Font("Verdana", Font.BOLD, 15));
			g2.drawString("Name: " + selectedPoke.getName(), statsX + 15, statsY + 20);
			g2.drawString("Type: " + selectedPoke.getType(), statsX + 15, statsY + 40);
			g2.drawString("Health: " + selectedPoke.getCurrentHP() + "/" + selectedPoke.getMaxHP(), statsX + 15,
					statsY + 60);
			g2.drawString("Run Odds: " + selectedPoke.getProbabilityToRun(), statsX + 15, statsY + 80);
		}
	...
	public Pokemon getPokemon(int x, int y) {
		...
		final int col = (int) (((double) relX / (double) this.pokedexSize.x) * cols);
		final int row = (int) (((double) relY / (double) this.pokedexSize.y) * rows);
	...
	private void drawStatsAndItems(Graphics2D g2) {
		...
		g2.drawImage(steps, x, y + 0 * (size + spacing), size, size, null, null);
		g2.drawString(stepsStr, x + size + spacing, y + 1 * (size + spacing) - 2 * spacing);

		Image hp = new ImageIcon(this.getClass().getResource(hpPNG)).getImage();
		g2.drawImage(hp, x, y + 1 * (size + spacing), size, size, null, null);
		g2.drawString(hpStr, x + size + spacing, y + 2 * (size + spacing) - 2 * spacing);

		Image safariball = new ImageIcon(this.getClass().getResource(safariballPNG)).getImage();
		g2.drawImage(safariball, x, y + 2 * (size + spacing), size, size, null, null);
		g2.drawString(safariballStr, x + size + spacing, y + 3 * (size + spacing) - 2 * spacing);

		Image potion = new ImageIcon(this.getClass().getResource(potionPNG)).getImage();
		g2.drawImage(potion, x, y + 3 * (size + spacing), size, size, null, null);
		g2.drawString(potionStr, x + size + spacing, y + 4 * (size + spacing) - 2 * spacing);

		Image snacks = new ImageIcon(this.getClass().getResource(snacksPNG)).getImage();
		g2.drawImage(snacks, x, y + 4 * (size + spacing), size, size, null, null);
		g2.drawString(snacksStr, x + size + spacing, y + 5 * (size + spacing) - 2 * spacing);
	}
	...
	public void on() {
		...
		safariballStr = "x " + model.getPlayer().getMyBag().getNumItem("Safari Balls");
